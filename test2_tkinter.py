...
# Store the reference to the image so it doesn't get garbage collected
    ...
    # This method will run modbus2.py in the background
        def run_modbus2():
            print(f"function calledss")
            try:
                # Source the environment setup script and run modbus2.py with the required arguments
                command = "python3 basic_pipelines/modbus2.py -i rpi --hef-path yolov8n2try.hef --labels config.json --arch hailo8l"
                result = subprocess.run(command, shell=True, check=True, executable="/bin/bash")
                print("Output:", result.stdout.decode())
                print("Error:", result.stderr.decode())
                print("function called")
            except Exception as e:
                print(f"Error running modbus2.py: {e}")

        # Create a new thread to run the barcode detection without blocking the GUI
        detection_thread = threading.Thread(target=run_modbus2)
        detection_thread.daemon = True  # This ensures the thread will exit when the main program exits
