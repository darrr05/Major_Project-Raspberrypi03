...
WEBCAM_INDEX = 16  # Change this if your webcam is not at index 0
...
class SupermarketApp(tk.Tk):
    ...
    def about_system(self):
        ...
        content = (
            "⌛ Platform: General PC with Webcam\n"
    ...
    def start_detection(self):
        ...
        try:
            ...
            print(f"Webcam initialized successfully at index {WEBCAM_INDEX}")
        except Exception as e:
            print(f"Failed to initialize webcam: {e}")
            messagebox.showerror("Error", f"Cannot initialize webcam: {e}")
            self.running = False
            self.webcam = None
    ...
    def detection_loop(self):
        while self.running:
            if not self.webcam:
                print("Webcam not initialized in detection loop")
                break
            ...
            if USE_FIXED_ROI:
                ...
                if det['label'] != 'number' or det['conf'] <= 0.5:
                    print("Invalid detection: not a number or low confidence")
                    continue
    ...
    def live_camera(self):
        ...
        if not self.running or not self.webcam:
            messagebox.showerror("Error", "Cannot open webcam. Ensure it is connected and not in use by another application.")
            return
        ...
        def update_frame():
            if not self.live_cam_running or not self.live_cam_win.winfo_exists():
                return
            if not self.webcam:
                print("Webcam not available in update_frame")
                return

            ret, frame = self.webcam.read()
            if not ret or frame is None:
                print("Failed to capture frame in update_frame")
                self.dome_img_label.config(image='')
                self.raw_img_label.config(image='')
                self.proc_img_label.config(image='')
                self.live_cam_win.after(30, update_frame)
                return

            print("Frame captured in update_frame")
