...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.dome_debounce_interval = 2  # Seconds; adjust based on testing
        ...
        self.ribbon_adjust_debounce = 2  # Debounce interval for ribbon adjustments
        self.mismatch_count = tk.IntVar(value=0)
        self.last_mismatch_state = None  # Track the last mismatch state (tuple of dome, ribbon counts)
        self.last_barcode_time = 0
        self.barcode_timeout = 30
                ...
                if msg.topic == MQTT_TOPIC_BARCODE:
                    ...
                    if text_key == "barcode_detected":
                        with self.number_lock:
                            ...
                            if current_time - self.last_barcode_increment_time >= self.barcode_debounce_interval:
                                self.last_barcode_increment_time = current_time
                                current_count = self.ribbon_count.get()
                                self.ribbon_count.set(current_count + 1)
                                print(f"Incremented ribbon_count to {self.ribbon_count.get()}")
                                self.mqtt_client.publish(MQTT_TOPIC_SUMMARY, str(self.ribbon_count.get()))
                                print(f"Published ribbon_count={self.ribbon_count.get()} to {MQTT_TOPIC_SUMMARY}")
                    current_time = time.time()
                    ...
                    with self.number_lock:
                        try:
                            ...
                            if count > dome:
                                ...
                                print(self.dome_count.get())
                                print("Dome count following summary!")
                            if count == 0:
    ...
    def start(self):
        ...
        def update_counts():
            ...
            with self.number_lock:
                ...
                current_time = time.time()
                # Check for barcode timeout to reset ribbon_count
                if ribbon > 0 and (current_time - self.last_barcode_time) >= self.barcode_timeout:
                    self.ribbon_count.set(0)
                    self.confirmed_number = "0"
                    self.last_confirmed_number = "0"
                    self.mismatch_count.set(0)  # Reset mismatch_count
                    self.camera_block_count.set(0)  # Reset camera_block_count
                    self.mqtt_client.publish(MQTT_TOPIC_SUMMARY, "0")
                    print("Ribbon count reset to 0 due to 30-second barcode timeout")
                    try:
                        conn = mariadb.connect(**DB_CONFIG)
                        cursor = conn.cursor()
                        cursor.execute(
                            "INSERT INTO activity_log (timestamp, description, image_path) VALUES (NOW(), ?, NULL)",
                            ("Ribbon count reset to 0 due to 30-second barcode timeout, mismatch and camera block counts reset",)
                        )
                        conn.commit()
                        conn.close()
                        print("Logged ribbon count reset to activity_log")
                    except mariadb.Error as err:
                        print(f"Error logging ribbon count reset to activity_log: {err}")
                # Mismatch logic
                if dome != ribbon:
                    if self.mismatch_start_time is None:
                        self.mismatch_start_time = current_time
                    current_mismatch_state = (dome, ribbon)
                    if (current_time - self.mismatch_start_time) >= 4:
        ...
        def after_count_init():
            if not live_counts_win.winfo_exists():
                return
            # Initialize flash_screen if not already created or destroyed
            ...
            target_width, target_height = 300, 300  # Reduced size for lower memory usage
            frame_skip = 10  # Skip more frames for faster loading
            ...
            def load_frames(img, new_size, key):
                try:
                    ...
                    if not gif_frames[key]:
                        print(f"No frames loaded for {key}")
            ...
            def load_all_gifs():
                for key, path in gif_paths.items():
                    try:
                        ...
                        print(f"Failed to load GIF {key}: {e}")
                        flash_image_label.config(text=f"Failed to load {key}", fg="red")

            # Load GIFs in a separate thread with error handling
            ...
            def animate_gif(label, frames):
                if not frames or not label.winfo_exists():
                    print("No frames or label destroyed, stopping animation")
                    return
                count = [0]
                def update_frame():
                    if not label.winfo_exists() or not frames:
                        print("Label destroyed or no frames, stopping animation")
                        return
                    try:
                        label.config(image=frames[count[0]])
                        label.image = frames[count[0]]  # Keep reference
                        count[0] = (count[0] + 1) % len(frames)
                        self.current_animation = label.after(100, update_frame)  # Slower animation (100ms)
                    except Exception as e:
            ...
            def flash_green_tick(text_key, frames):
                if not frames:
                    print(f"No frames for {text_key}")
                    return
                with self.flash_lock:
                    if not self.flash_screen.winfo_exists():
                        print("Flash screen destroyed, recreating")
                        self.flash_screen = tk.Toplevel(self)
                        self.flash_screen.withdraw()
                        self.flash_screen.configure(bg="white")
                        self.flash_screen.attributes("-topmost", True)
                    ...
                    self.flash_screen.after(2000, lambda: [
                        self.flash_screen.withdraw(),
            ...
            def flash_event(text_key):
                if not live_counts_win.winfo_exists():
                    print("Live counts window closed, skipping flash")
                    return
                frames = gif_frames.get(text_key, [])
                print(f"Flashing for {text_key}, frames available: {len(frames)}")
                if text_key == "barcode_detected":
        ...
        def on_live_counts_close():
            ...
            with self.number_lock:
                ...
                self.camera_block_count.set(0)  # Reset camera_block_count
                self.mqtt_client.publish(MQTT_TOPIC_SUMMARY, "0")
                print("Ribbon count reset to 0 on live counts close")
                try:
                    conn = mariadb.connect(**DB_CONFIG)
                    cursor = conn.cursor()
                    cursor.execute(
                        "INSERT INTO activity_log (timestamp, description, image_path) VALUES (NOW(), ?, NULL)",
                        ("Ribbon count reset to 0 on live counts window close, mismatch and camera block counts reset",)
                    )
                    conn.commit()
                    conn.close()
                    print("Logged ribbon count reset on close to activity_log")
                except mariadb.Error as err:
                    print(f"Error logging ribbon count reset on close: {err}")
        ...
        # Add mismatch counter at bottom left
