...
translations = {
    "en": {
        ...
        "image": "Image",
        "exit": "Exit",
        "mismatch_alert": "⚠️ Item Mismatch Detected! Please ensure all scanned items are placed in the bagging area."
    },
    "zh": {
        "settings": "设置",
        "volume": "扬声器音量：",
        ...
        "image": "图片",
        "exit": "退出",
        "mismatch_alert": "⚠️ 检测到物品不匹配！请确保所有扫描的物品都放入装袋区域。"
    }
}
...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.dome_count = tk.IntVar(value=0)  # Initialize dome count
    ...
    def start_mqtt(self):
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                print(f"Invalid MQTT message received: {msg.payload.decode()}")
    ...
    def show_mismatch_alert(self, live_counts_win, resume_flash_callback):
        border_color = "white" if self.theme == "dark" else "black"
        alert_win = tk.Toplevel(live_counts_win)
        alert_win.grab_set()
        alert_win.resizable(False, False)
        alert_win.overrideredirect(True)
        alert_win.configure(bg=border_color)
        ...
        border_frame = tk.Frame(alert_win, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)

        inner_frame = tk.Frame(border_frame, bg="#ff0000")
        inner_frame.pack(expand=True, fill="both")

        def flash_background():
            if not alert_win.winfo_exists():
                return
            current_bg = inner_frame.cget("bg")
            new_bg = "#cc0000" if current_bg == "#ff0000" else "#ff0000"
            inner_frame.config(bg=new_bg)
            alert_label.config(bg=new_bg)
            alert_win.after(500, flash_background)

        alert_label = tk.Label(
            inner_frame,
            text=self.tr("mismatch_alert"),
        ...
        ok_btn = tk.Button(
            ...
            command=lambda: [alert_win.destroy(), resume_flash_callback()],
            relief="raised",
            bd=3
        )
        ok_btn.pack(pady=20)
        ...
        alert_win.bind("<Return>", lambda e: [alert_win.destroy(), resume_flash_callback()])

        flash_background()
        print("Playing alert sound (simulated)")
        ...
        def after_count_init():
            ...
            def flash_mismatch_alert():
                ...
                try:
                    ...
                    cursor = conn.cursor()
                    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    description = f"Item Mismatch: Scanned {self.dome_count.get()}, Bagged {ribbon_count.get()}"
                    image_path = ""
                    cursor.execute(
                        "INSERT INTO activity_log (timestamp, description, image_path) VALUES (%s, %s, %s)",
                        (timestamp, description, image_path)
                    ...
                    print(f"Failed to log mismatch: {err}")
                self.show_mismatch_alert(live_counts_win, simulate_flash_events)
            ...
            def simulate_flash_events():
                ...
                if self.dome_count.get() != ribbon_count.get():
                    events.append(flash_mismatch_alert)

                func = random.choice(events)
                func()
                if func != flash_mismatch_alert:  # Only reschedule if not mismatch alert
