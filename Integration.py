...
class SupermarketApp(tk.Tk):
    ...
    # ~ def live_camera(self):
        ...
        # ~ dome_title = tk.Label(inner_frame, text=self.tr("Barcode Camera Feed"), font=("Times New Roman", 20, "bold"),
                            # ~ bg=self.bg_color, fg=self.fg_color)
        # ~ dome_title.pack(pady=(10, 5))
        ...
        # ~ ribbon_title = tk.Label(inner_frame, text=self.tr("Number Camera Feed"), font=("Times New Roman", 20, "bold"),
                                # ~ bg=self.bg_color, fg=self.fg_color)
        # ~ ribbon_title.pack(pady=(0, 5))
        ...
        # ~ def update_frame():
            ...
            # ~ if frame is not None:
                # ~ # Dome camera preview
                # ~ dome_resized = cv2.resize(frame, (320, 240))
                # ~ dome_rgb = cv2.cvtColor(dome_resized, cv2.COLOR_BGR2RGB)
                # ~ dome_img = Image.fromarray(dome_rgb)
                # ~ dome_imgtk = ImageTk.PhotoImage(image=dome_img)
                # ~ self.dome_img_label.imgtk = dome_imgtk
                ...
                # ~ # Ribbon camera with ROI
                # ~ frame_resized = cv2.resize(frame, (320, 240))

                # ~ # Dummy ROI centered
                # ~ h, w = frame_resized.shape[:2]
                # ~ box_w, box_h = w // 8, h // 10
                # ~ x1 = w // 2 - box_w // 2 - PADDING_X
                # ~ y1 = h // 2 - box_h // 2 - PADDING_Y
                # ~ x2 = w // 2 + box_w // 2 + PADDING_X
                # ~ y2 = h // 2 + box_h // 2 + PADDING_Y

                # ~ # Draw rectangle
                # ~ cv2.rectangle(frame_resized, (x1, y1), (x2, y2), (0, 255, 0), 2)

                # ~ # Show raw image with ROI
                # ~ frame_rgb = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
                # ~ img_raw = Image.fromarray(frame_rgb)
                # ~ imgtk_raw = ImageTk.PhotoImage(image=img_raw)
                # ~ self.raw_img_label.imgtk = imgtk_raw
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                if msg.topic == MQTT_TOPIC_COUNTER:
                    ...
                    print(f"Received MQTT barcode message: {message}")
                    message_map = {
                    ...
                    if message in message_map and hasattr(self, 'flash_event'):
                        self.flash_event(message_map[message])
    ...
    def start(self):
        ...
        def update_counts():
            try:
                ...
                detected_num = 0
            self.ribbon_count.set(detected_num)
        ...
        def after_count_init():
            ...
            # Load GIF images
            barcode_blocked_img = Image.open("/home/pi/hailo-rpi5-examples/images/BarcodeBlocked.gif")
            multiple_barcode_img = Image.open("/home/pi/hailo-rpi5-examples/images/MultipleBarcode.gif")
            camera_blocked_img = Image.open("/home/pi/hailo-rpi5-examples/images/CameraBlocked.gif")
            barcode_detected_img = Image.open("/home/pi/hailo-rpi5-examples/images/BarcodeDetected.gif")
            ...
            def get_frames(img):
                ...
                try:
                    ...
                    pass  # No more frames in the GIF
                return frames

            # Extract frames for each GIF
            barcode_blocked_frames = get_frames(barcode_blocked_img)
            multiple_barcode_frames = get_frames(multiple_barcode_img)
            camera_blocked_frames = get_frames(camera_blocked_img)
            barcode_detected_frames = get_frames(barcode_detected_img)

            def animate_gif(label, frames, interval=100):
                def update_frame(frame_index=0):
                    label.config(image=frames[frame_index])
                    frame_index = (frame_index + 1) % len(frames)
                    label.after(interval, update_frame, frame_index)

                update_frame()

            def flash_green_tick(text_key, img_frames):
                self.flash_screen.configure(bg="#00ff00")
                flash_label.config(text=self.tr(text_key),
                                font=("Times New Roman", 48, "bold"),
                                fg="white", bg="#00ff00")
                flash_image_label.config(bg="#00ff00")
                animate_gif(flash_image_label, img_frames)  # Start animation
            ...
            def flash_red_screen(text_key, img_frames):
            ...
            def flash_event(text_key):
                ...
                if text_key == "barcode_detected":
                    flash_green_tick(text_key, barcode_detected_frames)
                elif text_key in ["barcode_blocked", "multiple_barcodes", "camera_blocked"]:
                    image_map = {
                        "barcode_blocked": barcode_blocked_frames,
                        "multiple_barcodes": multiple_barcode_frames,
                        "camera_blocked": camera_blocked_frames
                    }
                    flash_red_screen(text_key, image_map[text_key])
