...
class SupermarketApp(tk.Tk):
        ...
        for widget in self.top_bar.winfo_children():
            if isinstance(widget, tk.Button):
                if widget == self.shutdown_btn:
                    ...
                    widget.config(bg=self.top_bar_bg, fg=self.fg_color if self.theme == "dark" else "black", activebackground="#335533" if self.theme == "dark" else "#d6f5 conglomerated")
    ...
    def create_widgets(self):
        ...
        shutdown_path = os.path.expanduser("/home/pi/hailo-rpi5-examples/images/pb.png")
    ...
    def start_detection(self):
        if self.running:
            return
        try:
            self.dome_cam = Picamera2(0)  # Dome camera
            self.ribbon_cam = Picamera2(1)  # Ribbon camera
            self.dome_cam.configure(self.dome_cam.create_preview_configuration(main={"format": "RGB888"}))
            self.ribbon_cam.configure(self.ribbon_cam.create_preview_configuration(main={"format": "RGB888"}))
            self.dome_cam.start()
            self.ribbon_cam.start()
            self.running = True
        except Exception as e:
            messagebox.showerror("Error", f"Cannot initialize cameras: {e}")
            self.dome_cam = None
            self.ribbon_cam = None

    def stop_detection(self):
        self.running = False
        if self.dome_cam is not None:
            self.dome_cam.stop()
            self.dome_cam.close()
            self.dome_cam = None
        if self.ribbon_cam is not None:
            self.ribbon_cam.stop()
            self.ribbon_cam.close()
            self.ribbon_cam = None
    ...
    def live_camera(self):
        if not self.dome_cam or not self.ribbon_cam or not self.running:
            self.start_detection()
            if not self.dome_cam or not self.ribbon_cam or not self.running:
                tk.messagebox.showerror("Error", "Cannot initialize cameras")
                return
        border_color = "white" if self.theme == "dark" else "black"
        width, height = 1000, 600
        ...
        inner_frame.pack(expand=True, fill="both")
        # Dome Camera Feed
        dome_title = tk.Label(inner_frame, text=self.tr("Dome Camera Feed"), font=("Times New Roman", 16, "bold"),
                             bg=self.bg_color, fg=self.fg_color)
        ...
        # Side-by-Side Barcode and Number Detection
        side_by_side_frame = tk.Frame(inner_frame, bg=self.bg_color)
        side_by_side_frame.pack(pady=(0, 20))
        # Barcode Detection Camera (Left)
        barcode_container = tk.Frame(side_by_side_frame, bg=self.bg_color)
        barcode_container.pack(side=tk.LEFT, padx=20)
        barcode_title = tk.Label(barcode_container, text="Barcode Detection", font=("Times New Roman", 14, "bold"),
                                bg=self.bg_color, fg=self.fg_color)
        barcode_title.pack(pady=(0, 5))
        self.raw_img_label = tk.Label(barcode_container, bg=self.bg_color)
        self.raw_img_label.pack()
        # Number Detection Camera (Right)
        number_container = tk.Frame(side_by_side_frame, bg=self.bg_color)
        number_container.pack(side=tk.RIGHT, padx=20)
        number_title = tk.Label(number_container, text="Number Detection", font=("Times New Roman", 14, "bold"),
                               bg=self.bg_color, fg=self.fg_color)
        number_title.pack(pady=(0, 5))
        self.proc_img_label = tk.Label(number_container, bg=self.bg_color)
        self.proc_img_label.pack()
        # Exit Button
        cancel_btn = tk.Button(inner_frame, text=self.tr("exit"), font=("Times New Roman", 12, "bold"),
                              bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg,
                              relief="raised", bd=2, width=12, command=lambda: on_close())
        ...
        def update_frame():
            if not self.live_cam_running or not self.live_cam_win.winfo_exists():
                return
            # Dome camera feed
            dome_frame = self.dome_cam.capture_array()
            if dome_frame is not None:
                dome_resized = cv2.resize(dome_frame, (640, 250))
                if USE_FIXED_ROI:
                    x1, y1, x2, y2 = ROI_X1, ROI_Y1, ROI_X2, ROI_Y2
                else:
                    detections = self.dummy_yolo_detector(dome_resized)
                    if detections:
                        det = detections[0]
                        x1, y1, x2, y2 = det['bbox']
                        x1 = max(0, x1 - PADDING_X)
                        y1 = max(0, y1 - PADDING_Y)
                        x2 = min(dome_resized.shape[1], x2 + PADDING_X)
                        y2 = min(dome_resized.shape[0], y2 + PADDING_Y)
                    else:
                        x1, y1, x2, y2 = 0, 0, dome_resized.shape[1], dome_resized.shape[0]
                cv2.rectangle(dome_resized, (x1, y1), (x2, y2), (0, 255, 0), 2)
            ...
            # Barcode Detection Camera (Ribbon camera feed)
            ribbon_frame = self.ribbon_cam.capture_array()
            if ribbon_frame is not None:
                ribbon_resized = cv2.resize(ribbon_frame, (300, 230))
                if USE_FIXED_ROI:
                    x1, y1, x2, y2 = ROI_X1, ROI_Y1, ROI_X2, ROI_Y2
                else:
                    detections = self.dummy_yolo_detector(ribbon_resized)
                    if detections:
                        det = detections[0]
                        x1, y1, x2, y2 = det['bbox']
                        x1 = max(0, x1 - PADDING_X)
                        y1 = max(0, y1 - PADDING_Y)
                        x2 = min(ribbon_resized.shape[1], x2 + PADDING_X)
                        y2 = min(ribbon_resized.shape[0], y2 + PADDING_Y)
                    else:
                        x1, y1, x2, y2 = 0, 0, ribbon_resized.shape[1], ribbon_resized.shape[0]
                cv2.rectangle(ribbon_resized, (x1, y1), (x2, y2), (0, 255, 0), 2)
                ribbon_rgb = cv2.cvtColor(ribbon_resized, cv2.COLOR_BGR2RGB)
                raw_img = Image.fromarray(ribbon_rgb)
                raw_imgtk = ImageTk.PhotoImage(image=raw_img)
                self.raw_img_label.imgtk = raw_imgtk
                self.raw_img_label.config(image=raw_imgtk)
            # Number Detection Camera (Dome camera feed)
            if dome_frame is not None:
                dome_resized = cv2.resize(dome_frame, (300, 230))
                if USE_FIXED_ROI:
                    x1, y1, x2, y2 = ROI_X1, ROI_Y1, ROI_X2, ROI_Y2
                else:
                    detections = self.dummy_yolo_detector(dome_resized)
                ...
                cv2.rectangle(dome_resized, (x1, y1), (x2, y2), (0, 255, 0), 2)
                dome_rgb = cv2.cvtColor(dome_resized, cv2.COLOR_BGR2RGB)
                proc_img = Image.fromarray(dome_rgb)
                proc_imgtk = ImageTk.PhotoImage(image=proc_img)
                self.proc_img_label.imgtk = proc_imgtk
                self.proc_img_label.config(image=proc_imgtk)
    ...
    def start(self):
        ...
        def update_counts():
            ...
            with self.number_lock:
                ribbon_count.set(self.ribbon_count.get())
        ...
        def after_count_init():
            ...
            test_btn = tk.Button(live_counts_win, text="Test Flash", font=("Times New Roman", 14),
                                 command=lambda: self.flash_event("barcode_blocked"),
