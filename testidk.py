...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.ribbon_count = tk.IntVar(value=0)  # Initialize ribbon count
    ...
    def activity_log(self):
        ...
        tree.column("ID", width=0, stretch=False)  # Hide ID
    ...
    def start(self):
        self.start_mqtt()
        self.prompt_active = False  # Track if password prompt is active
        ...
        def after_count_init():
            ...
            flash_screen.geometry(f"{flash_screen.winfo_screenwidth()}x{flash_screen.winfo_screenheight()}")
            #flash_screen.overrideredirect(True)
            ...
            barcode_blocked_img = ImageTk.PhotoImage(barcode_blocked_img.resize((barcode_blocked_img.width // 2, barcode_blocked_img.height // 2)))
            multiple_barcode_img = ImageTk.PhotoImage(multiple_barcode_img.resize((multiple_barcode_img.width // 2, multiple_barcode_img.height // 2)))
            camera_blocked_img = ImageTk.PhotoImage(camera_blocked_img.resize((camera_blocked_img.width // 2, camera_blocked_img.height // 2)))
            barcode_detected_img = ImageTk.PhotoImage(barcode_detected_img.resize((barcode_detected_img.width // 2, barcode_detected_img.height // 2)))
            ...
            def flash_green_tick(text, img):
                ...
                if not self.prompt_active:
                    flash_screen.lift()
                    # flash_screen.focus_force()  # Removed to prevent interrupting password input
                flash_screen.after(1000, lambda: [
                    flash_screen.withdraw(),
                    live_counts_win.lift(),
                    None if self.prompt_active else live_counts_win.focus_force()
                ])
        ...
        def exit_fullscreen():
            ...
            def check_password():
                if password_entry.get() == HARDCODED_PASSWORD:
                    ...
                    live_counts_win.destroy()
                    self.stop_mqtt()
                    if hasattr(self, "flash_screen") and self.flash_screen.winfo_exists():
            ...
            def show_onboard_keyboard(event=None):
                subprocess.Popen(["onboard"])

            password_entry.bind("<FocusIn>", show_onboard_keyboard)
            ...
            tk.Button(buttons_frame, text=self.tr("cancel"), font=("Times New Roman", 12, "bold"),
                    bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, bd=2,
                    command=lambda: [prompt_win.destroy(), setattr(self, "prompt_active", False)]).pack(side=tk.LEFT, padx=10)

            prompt_win.bind("<Return>", lambda e: check_password())
            prompt_win.protocol("WM_DELETE_WINDOW", lambda: subprocess.run(["pkill", "onboard"]))
