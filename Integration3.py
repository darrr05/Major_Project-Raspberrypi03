...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.barcode_process = None  # To store the subprocess
    ...
    def start(self):
        # Start the barcode detection script in the background
        script_path = os.path.expanduser("~/hailo-rpi5-examples/basic_pipelines/barcode_detection.py")
        venv_path = os.path.expanduser("~/hailo-rpi5-examples/venv_hailo_rpi5_examples/bin/activate")
        hef_path = os.path.expanduser("~/hailo-rpi5-examples/yolov8n2try.hef")
        labels_path = os.path.expanduser("~/hailo-rpi5-examples/labels.json")
        working_dir = os.path.expanduser("~/hailo-rpi5-examples/basic_pipelines")
        
        if not all(os.path.exists(path) for path in [venv_path, script_path, hef_path, labels_path]):
            messagebox.showerror("Error", "Required files for barcode detection not found.")
            return

        try:
            # Construct command to activate virtual environment and run the script
            cmd = f". {venv_path} && python {script_path} -i rpi --hef-path {hef_path} --labels {labels_path}"
            self.barcode_process = subprocess.Popen(
                cmd,
                shell=True,
            ...
            print(f"Started barcode detection process with PID {self.barcode_process.pid}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to start barcode detection: {e}")
            self.barcode_process = None
            return
        ...
        def after_count_init():
            ...
            self.flash_screen.attributes('-fullscreen', True)
            ...
            try:
                barcode_blocked_img = tk.PhotoImage(file=os.path.expanduser("~/Downloads/barcode_blocked.png")).subsample(2, 2)
                multiple_barcode_img = tk.PhotoImage(file=os.path.expanduser("~/Downloads/multiple_barcode.png")).subsample(2, 2)
                camera_blocked_img = tk.PhotoImage(file=os.path.expanduser("~/Downloads/camera_blocked.png")).subsample(2, 2)
                barcode_detected_img = tk.PhotoImage(file=os.path.expanduser("~/Downloads/barcode_detected.png")).subsample(2, 2)
            except tk.TclError as e:
                print(f"Error loading images: {e}")
                barcode_blocked_img = multiple_barcode_img = camera_blocked_img = barcode_detected_img = None
            ...
            def on_live_counts_close():
                ...
                if self.barcode_process is not None:
                    try:
                        self.barcode_process.terminate()
                        self.barcode_process.wait(timeout=2)
                        print(f"Terminated barcode detection process with PID {self.barcode_process.pid}")
                    except subprocess.TimeoutExpired:
                        self.barcode_process.kill()
                        print(f"Forcibly killed barcode detection process with PID {self.barcode_process.pid}")
                    self.barcode_process = None
