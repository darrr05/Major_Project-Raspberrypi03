...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.mqtt_client.connect("localhost", 1883, 60)  # Replace with IP if needed
...
# Store the reference to the image so it doesn't get garbage collected
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        client.subscribe("barcode/detection")

    def on_mqtt_message(self, client, userdata, msg):
        label = msg.payload.decode()
        print(f"Received MQTT message: {label}")

        if label == "have_barcode":
            self.flash_screen(self.tr("barcode_detected"), self.barcode_detected_img, "#00ff00")
        elif label == "blocked_barcode":
            self.flash_screen(self.tr("barcode_blocked"), self.barcode_blocked_img, "#ff0000")
        elif label == "multiple_barcodes":
            self.flash_screen(self.tr("multiple_barcodes"), self.multiple_barcode_img, "#ff0000")
        elif label == "camera_blocked":
            self.flash_screen(self.tr("camera_blocked"), self.camera_blocked_img, "#ff0000")
        else:
            print("Unknown label received:", label)

    def show_image(self, label_key):
        images = {
            "barcode_detected": "barcode_detected_img",  # Replace with actual image paths
            "barcode_blocked": "barcode_blocked_img",   # Replace with actual image paths
        }

        image_path = os.path.expanduser(images.get(label_key, ""))
    ...
    def start(self):
        ...
        def flash_screen(self, text, img, color):
            # Create a new window to show the flash screen
            flash_screen = tk.Toplevel(self)
            flash_screen.grab_set()
            flash_screen.attributes('-fullscreen', True)
            flash_screen.configure(bg=color)

            # Display the text in large font
            flash_label = tk.Label(flash_screen, text=text, font=("Times New Roman", 48, "bold"), fg="white", bg=color)
            flash_label.pack(pady=50)

            # Display the image in the flash screen if available
            if img:
                flash_image_label = tk.Label(flash_screen, bg=color)
                flash_image_label.pack()

                img_resized = img.resize((img.width // 2, img.height // 2))  # Resize the image if needed
                img_tk = ImageTk.PhotoImage(img_resized)
                flash_image_label.imgtk = img_tk
                flash_image_label.config(image=img_tk)

            # Close the flash screen after 1 second
            flash_screen.after(1000, lambda: flash_screen.destroy())
