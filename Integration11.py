...
translations = {
    "en": {
        ...
        "mismatch_alert": "⚠️ Count Mismatch Detected",
        "camera_block_alert": "⚠️ Camera Block Detected",
        "mismatch_message": "Mismatch detected: Dome Camera Count ({dome}) does not match Ribbon Camera Count ({ribbon})."
    },
    "zh": {
        "settings": "设置",
        "volume": "扬声器音量：",
        ...
        "object_count": "物件数目",
        "number_count": "数目统计",
        "mismatch_alert": "⚠️ 计数不匹配警报",
        "camera_block_alert": "⚠️ 摄像头遮挡警报",
        "mismatch_message": "检测到不匹配：球形摄像头计数 ({dome}) 与条码摄像头计数 ({ribbon}) 不一致。"
    }
}
...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        self.camera_block_count = tk.IntVar(value=0)
        ...
        self.last_barcode_increment_time = 0
        self.dome_debounce_interval = 2
        self.last_dome_increment_time = 0
        self.recent_dome_counts = deque(maxlen=3)
        self.barcode_debounce_interval = 0.0
        self.mismatch_start_time = None
        self.mismatch_popup_shown = False
        self.last_ribbon_adjust_time = 0
        self.ribbon_adjust_debounce = 2
        self.mismatch_count = tk.IntVar(value=0)
    ...
    def open_settings(self):
        ...
        save_btn = tk.Button(buttons_frame, text=self.tr("save"), font=("Times New Roman", 12, "bold"),
                            bg=self.fg_color, fg=self.bg_color, activebackground=self.btn_active_bg,
                            relief="raised", bd=3, command=save_settings)
    ...
    def about_system(self):
        ...
        close_btn = tk.Button(inner_frame, text="Close", font=("Times New Roman", 12, "bold"), bg="#007c30", fg="white", padx=10, pady=5,
                             activebackground="#005c24", relief="flat", command=about_win.destroy)
    ...
    def show_mismatch_popup(self, live_counts_win, dome, ribbon):
        if not live_counts_win.winfo_exists() or not live_counts_win.winfo_ismapped():
            return
        focused_widget = live_counts_win.focus_displayof()
        if focused_widget is None or focused_widget.winfo_toplevel() != live_counts_win:
            return
        border_color = "white" if self.theme == "dark" else "black"
        popup_win = tk.Toplevel(live_counts_win)
        popup_win.transient(live_counts_win)
        popup_win.grab_set()
        popup_win.resizable(False, False)
        popup_win.overrideredirect(True)
        popup_win.configure(bg=border_color)
        width, height = 350, 150
        win_x = live_counts_win.winfo_rootx() + (live_counts_win.winfo_width() // 2) - (width // 2)
        win_y = live_counts_win.winfo_rooty() + 50  # Position at top
        popup_win.geometry(f"{width}x{height}+{win_x}+{win_y}")
        border_frame = tk.Frame(popup_win, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("mismatch_alert"), font=("Times New Roman", 14, "bold"),
                        bg=self.bg_color, fg="red")
        title.pack(pady=(10, 5))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 5))
        message = self.tr("mismatch_message").format(dome=dome, ribbon=ribbon)
        tk.Label(inner_frame, text=message, font=("Times New Roman", 12), bg=self.bg_color, fg=self.text_fg, wraplength=300).pack(pady=5)
        popup_win.after(3000, popup_win.destroy)
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                if msg.topic == MQTT_TOPIC_BARCODE:
                    ...
                    if not text_key:
                        print(f"Unknown barcode message: {message}")
                        return
                    if text_key == "camera_blocked":
                        with self.number_lock:
                            self.camera_block_count.set(self.camera_block_count.get() + 1)
                    if not (hasattr(self, 'flash_screen') and self.flash_screen and self.flash_screen.winfo_exists()):
                        print("Flash screen not available or destroyed")
                        return
                    current_time = time.time()
                    with self.flash_lock:
                        if current_time - self.last_flash_time < self.flash_cooldown:
                            print(f"Flash skipped due to cooldown: {text_key}")
                            return
                        ...
                        try:
                            ...
                            try:
                                ...
                                if isinstance(data, dict):
                                    if "Objects Detected" in data:
                                        count = int(data["Objects Detected"])
                                    else:
                            ...
                            if count is None:
                                if raw.startswith("Object Count:"):
                                    ...
                                    count = val if val > 0 else 0
                            current_time = time.time()
                            if current_time - self.last_dome_increment_time < self.dome_debounce_interval:
                                if count > 1:
                                    count = 1
                                else:
                                    return
                            else:
                                self.recent_dome_counts.append(count)
                                if len(self.recent_dome_counts) > 1:
                                    avg_recent = sum(self.recent_dome_counts) / len(self.recent_dome_counts)
                                    if count > avg_recent * 1.5:
                                        count = max(1, int(avg_recent))
                            self.last_dome_increment_time = current_time
                            counter = self.dome_count.get()
                            self.dome_count.set(counter + count)
                        except Exception as e:
                            print(f"Error processing counter message: {e}")
                            self.dome_count_value = False
                    ...
                    with self.number_lock:
                        try:
                            count = int(message)
                            current_time = time.time()
                            if current_time - self.last_ribbon_adjust_time < self.ribbon_adjust_debounce:
                                return
                            self.last_ribbon_adjust_time = current_time
                            self.ribbon_count.set(count)
                            self.confirmed_number = str(count)
                            self.last_confirmed_number = self.confirmed_number
                            dome = self.dome_count.get()
                            if count > dome:
    ...
    def start(self):
        ...
        def update_counts():
            if not live_counts_win.winfo_exists():
                self.mismatch_start_time = None
                self.mismatch_count.set(0)
                self.camera_block_count.set(0)
                return
            with self.number_lock:
                dome = self.dome_count.get()
                ribbon = self.ribbon_count.get()
                current_time = time.time()
                if dome == 0 and ribbon == 0:
                    self.mismatch_count.set(0)
                    self.camera_block_count.set(0)
                    self.mismatch_start_time = None
                    self.last_mismatch_state = None
                    ...
                    current_mismatch_state = (dome, ribbon)
                    if (current_time - self.mismatch_start_time) >= 6:
                        if self.last_mismatch_state != current_mismatch_state:
                            self.mismatch_count.set(self.mismatch_count.get() + 1)
                            if dome > 0:
                                self.dome_count.set(dome - 1)
                                try:
                                    conn = mariadb.connect(**DB_CONFIG)
                                    cursor = conn.cursor()
                                    cursor.execute(
                                        "INSERT INTO activity_log (timestamp, description, image_path) VALUES (NOW(), ?, NULL)",
                                        (f"Dome count decremented by 1 due to mismatch: dome_count={dome-1}, ribbon_count={ribbon}",)
                                    )
                                    conn.commit()
                                    conn.close()
                                except mariadb.Error as err:
                                    print(f"Error logging dome count decrement: {err}")
                            self.last_mismatch_state = current_mismatch_state
                        try:
                            conn = mariadb.connect(**DB_CONFIG)
                            cursor = conn.cursor()
                            cursor.execute(
                                "INSERT INTO activity_log (timestamp, description, image_path) VALUES (NOW(), ?, NULL)",
                                (f"Mismatch detected: dome_count={self.dome_count.get()}, ribbon_count={ribbon}",)
                            )
                            conn.commit()
                            conn.close()
                            self.show_mismatch_popup(live_counts_win, self.dome_count.get(), ribbon)
                            self.mismatch_start_time = current_time
                        except mariadb.Error as err:
                            print(f"Error logging mismatch to activity_log: {err}")
                else:
                    self.mismatch_start_time = None
                    self.last_mismatch_state = None
        ...
        def after_count_init():
            if not live_counts_win.winfo_exists():
                return
            if not hasattr(self, 'flash_screen') or not self.flash_screen or not self.flash_screen.winfo_exists():
                ...
                self.flash_screen.withdraw()  # Hide initially

            flash_frame = tk.Frame(self.flash_screen, bg="white")
            flash_frame.pack(expand=True, fill="both", padx=20, pady=20)

            flash_image_label = tk.Label(flash_frame, bg="white")
            flash_image_label.pack(pady=(100, 20))

            flash_label = tk.Label(flash_frame, bg="white", font=("Times New Roman", 48, "bold"), fg="white")
            flash_label.pack()

            image_paths = {
                "barcode_detected": "/home/pi/hailo-rpi5-examples/images/barcode_detected.jpeg",
                "barcode_blocked": "/home/pi/hailo-rpi5-examples/images/barcode_blocked.jpeg",
                "camera_blocked": "/home/pi/hailo-rpi5-examples/images/camera_blocked.jpeg"
            }
            target_width, target_height = 450, 500
            images = {key: None for key in image_paths}
            ...
            def load_all_images():
                for key, path in image_paths.items():
                    try:
                        aspect_ratio = min(target_width / Image.open(path).width, target_height / Image.open(path).height)
                        new_size = (int(Image.open(path).width * aspect_ratio), int(Image.open(path).height * aspect_ratio))
                        images[key] = load_image(path, new_size)
                        if not images[key]:
            ...
            threading.Thread(target=load_all_images, daemon=True).start()

            def flash_green_tick(text_key):
                image = images.get(text_key)
                if not image:
                    print(f"No image available for {text_key}")
                    flash_image_label.config(text=f"No image for {text_key}", fg="red")
                    return
                with self.flash_lock:
                    print(f"Displaying static image for {text_key}")
                    self.flash_screen.configure(bg="#00ff00")
                    display_text = "✓ Barcode Detected" if text_key == "barcode_detected" else self.tr(text_key)
                    flash_label.config(text=display_text, fg="white", bg="#00ff00")
                    flash_image_label.config(bg="#00ff00", image=image)
                    flash_image_label.image = image
                    self.flash_screen.deiconify()
                    live_counts_win.lower()  # Ensure flash screen is on top
                    self.flash_screen.after(1500, self.flash_screen.withdraw)  # Auto-close after 3 seconds

            def flash_red_screen(text_key):
                image = images.get(text_key)
                if not image:
                    print(f"No image available for {text_key}")
                    flash_image_label.config(text=f"No image for {text_key}", fg="red")
                ...
                with self.flash_lock:
                    ...
                    flash_label.config(text=display_text, fg="white", bg="#ff0000")
                    flash_image_label.config(bg="#ff0000", image=image)
                    flash_image_label.image = image
                    self.flash_screen.deiconify()
                    live_counts_win.lower()
                    try:
                        conn = mariadb.connect(**DB_CONFIG)
                        cursor = conn.cursor()
                        cursor.execute(
            ...
            # Store flash functions for use in flash_event
            self.flash_green_tick = flash_green_tick
            self.flash_red_screen = flash_red_screen

        # Move flash_event to class level
        def flash_event(text_key):
            if not live_counts_win.winfo_exists() or not self.flash_screen or not self.flash_screen.winfo_exists():
                print("flash_event: Window does not exist, aborting")
                return
            print(f"flash_event: Triggering {text_key}")
            if text_key == "barcode_detected":
                self.flash_green_tick(text_key)
            elif text_key in ["barcode_blocked", "camera_blocked"]:
                self.flash_red_screen(text_key)
            else:
                print(f"Unknown text_key: {text_key}")

        # Set flash_event as an instance method
        ...
        mismatch_frame = tk.Frame(live_counts_win, bg="#e8f5e9")
        mismatch_frame.pack(side=tk.BOTTOM, anchor="sw", padx=20, pady=(0, 40))
        tk.Label(mismatch_frame, text=self.tr("mismatch_alert"), font=("Times New Roman", 14), bg="#e8f5e9", fg="red").pack(side=tk.LEFT)
        tk.Label(mismatch_frame, textvariable=self.mismatch_count, font=("Times New Roman", 14, "bold"), bg="#e8f5e9", fg="red").pack(side=tk.LEFT, padx=5)
        tk.Label(mismatch_frame, text=self.tr("camera_block_alert"), font=("Times New Roman", 14), bg="#e8f5e9", fg="red").pack(side=tk.LEFT, padx=20)
        tk.Label(mismatch_frame, textvariable=self.camera_block_count, font=("Times New Roman", 14, "bold"), bg="#e8f5e9", fg="red").pack(side=tk.LEFT, padx=5)
    ...
    def flash_event(self, text_key):
        if not hasattr(self, 'flash_screen') or not self.flash_screen or not self.flash_screen.winfo_exists():
            print("flash_event: Flash screen does not exist, aborting")
            return
        print(f"flash_event: Triggering {text_key}")
        if text_key == "barcode_detected":
            self.flash_green_tick(text_key)
