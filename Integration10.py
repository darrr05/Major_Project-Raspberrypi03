...
class SupermarketApp(tk.Tk):
    ...
    def show_mismatch_popup(self, live_counts_win, dome, ribbon):
        if not live_counts_win.winfo_exists() or not live_counts_win.winfo_ismapped():
            return
        # Check if live_counts_win or any of its children has focus
        ...
        win_y = live_counts_win.winfo_rooty() + (live_counts_win.winfo_height() // 2) - (height // 2)
        popup_win.geometry(f"{width}x{height}+{win_x}+{win_y}")
        border_frame = tk.Frame(popup_win, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("mismatch_alert"), font=("Times New Roman", 16, "bold"),
                        bg=self.bg_color, fg="red")
        title.pack(pady=(20, 10))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 10))
        message = self.tr("mismatch_message").format(dome=dome, ribbon=ribbon)
        tk.Label(inner_frame, text=message, font=("Times New Roman", 14), bg=self.bg_color, fg=self.text_fg, wraplength=350).pack(pady=10)
    ...
    def start_mqtt(self):
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                print(f"MQTT message received on {msg.topic}: {message}")  # Debug log
    ...
    def start(self):
        ...
        def update_counts():
            if not live_counts_win.winfo_exists():
                self.mismatch_start_time = None
                self.mismatch_count.set(0)  # Reset mismatch count when window closes
                return
            with self.number_lock:
                dome = self.dome_count.get()
                ribbon = self.ribbon_count.get()
                current_time = time.time()
                # Reset mismatch count if both dome and ribbon counts are 0
                if dome == 0 and ribbon == 0:
                    ...
                    # Increment mismatch count and decrement dome_count if this is a new mismatch state
                    if (current_time - self.mismatch_start_time) >= 7:
                        if self.last_mismatch_state != current_mismatch_state:
                            self.mismatch_count.set(self.mismatch_count.get() + 1)
                            # Decrement dome_count by 1, ensuring it doesn't go below 0
                            if dome > 0:
                    ...
                    self.mismatch_start_time = None
                    self.last_mismatch_state = None  # Reset mismatch state when counts match
        ...
        def after_count_init():
            ...
            def on_live_counts_close():
                ...
                self.mismatch_start_time = None
                self.mismatch_count.set(0)  # Reset mismatch count on close
                self.last_mismatch_state = None
