...
# MQTT Configuration
MQTT_BROKER = "192.168.252.60"
MQTT_PORT = 1883
MQTT_TOPIC_DOME = "counter/dome"
MQTT_TOPIC_RIBBON = "counter/ribbon"
...
translations = {
    "en": {
        ...
        "image": "Image",
        "exit": "Exit",
        "log_recorded": "Activity log recorded: Unusual behaviour detected",
        "mqtt_broker": "MQTT Broker IP:",
        "invalid_ip": "Invalid IP address format",
        "mqtt_error": "Failed to connect to MQTT broker. Check network and IP."
    },
    "zh": {
        "settings": "设置",
        "volume": "扬声器音量：",
        ...
        "image": "图片",
        "exit": "退出",
        "log_recorded": "活动记录已记录：检测到异常行为",
        "mqtt_broker": "MQTT经纪人IP：",
        "invalid_ip": "无效的IP地址格式",
        "mqtt_error": "无法连接到MQTT经纪人。请检查网络和IP。"
    }
}
...
class LoginDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        tk.Label(master, text=self.parent.tr("username"), font=("Times New Roman", 12)).grid(row=0, column=0, sticky="e", padx=5, pady=5)
        tk.Label(master, text=self.parent.tr("pass"), font=("Times New Roman", 12)).grid(row=1, column=0, sticky="e", padx=5, pady=5)
...
class SupermarketApp(tk.Tk):
        ...
        for widget in self.top_bar.winfo_children():
            if isinstance(widget, tk.Button):
                if widget == self.shutdown_btn:
                    widget.config(bg=self.top_bar_bg, fg=self.fg_color if self.theme == "dark" else "black", activebackground="#444444" if self.theme == "dark" else "#f0f0f0")
                    ...
                    if isinstance(subchild, tk.Label):
                        ...
                        for nested in subchild.winfo_children():
                            if isinstance(nested, tk.Label):
                                ...
                                nested.config(bg="white" if self.theme == "light" else "#333333", fg="black" if self.theme == "light" else "#dddddd", insertbackground=self.text_fg)
    ...
    def create_widgets(self):
        ...
        self.settings_btn = tk.Button(self.top_bar, text="⚙", font=("Times New Roman", 20), command=self.open_settings, bg="white", bd=0, activebackground="lightgray")
        ...
        self.shutdown_btn = tk.Button(self.top_bar, image=shutdown_img_tk, font=("Times New Roman", 20), command=self.prompt_shutdown_password, fg="black", bg="white", bd=0, activebackground="lightgray")
        self.shutdown_btn.image = shutdown_img_tk
        self.shutdown_btn.pack(side=tk.RIGHT, padx=(20, 10))

        self.logout_btn = tk.Button(self.top_bar, text=self.tr("logout"), font=("Times New Roman", 20), command=self.logout, fg="red", bg="white", bd=0, activebackground="lightgray")
        self.logout_btn.pack_forget()

        center_frame = tk.Frame(self.border_frame, bg="white")
        center_frame.pack(fill=tk.BOTH, expand=True, pady=2)

        logo_path = os.path.expanduser("/home/pi/hailo-rpi5-examples/images/giant_logo.jpeg")
        ...
        btn_style = {"width": 22, "height": 3, "font": btn_font, "bg": "#e8f5e9", "activebackground": "#c8e6c9", "bd": 2, "relief": "ridge"}
        ...
        self.start_btn = tk.Button(self.grid_frame, text=self.tr("start"), command=self.start, width=14, height=2, font=("Times New Roman", 12, "bold"), bg="#e8f5e9", activebackground="#c8e6c9", bd=2, relief="ridge")
        self.start_btn.grid(row=2, column=0, columnspan=2, pady=20)

        self.simulate_log_btn = tk.Button(self.grid_frame, text="Simulate Log", command=self.simulate_activity_log, width=14, height=2, font=("Times New Roman", 12, "bold"), bg="#e8f5e9", activebackground="#c8e6c9", bd=2, relief="ridge")
        self.simulate_log_btn.grid(row=2, column=1, pady=20)

        self.create_tip_box(center_frame)

        self.status_label = tk.Label(self.border_frame, text="", bg="white", fg="#007c30", font=("Times New Roman", 12, "italic"))
    ...
    def start_barcode_detection(self):
        def run_modbus2():
            try:
                command = "python3 basic_pipelines/modbus2.py -i rpi --hef-path yolov8n2try.hef --labels labels.json"
                result = subprocess.run(command, shell=True, check=True, executable="/bin/bash", capture_output=True, text=True)
                print("Output:", result.stdout)
                print("Error:", result.stderr)
            except Exception as e:
                messagebox.showerror("Error", f"Error running modbus2.py: {e}")
        detection_thread = threading.Thread(target=run_modbus2)
    ...
    def simulate_activity_log(self):
        try:
            ...
            image_path = ""
            cursor.execute("INSERT INTO activity_log (timestamp, description, image_path) VALUES (%s, %s, %s)", (timestamp, description, image_path))
            conn.commit()
            conn.close()
    ...
    def open_settings(self):
        ...
        title = tk.Label(inner_frame, text=self.tr("settings"), font=("Times New Roman", 20, "bold"), bg=self.bg_color, fg=self.fg_color)
        title.pack(pady=(20, 10))

        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 10))

        tk.Label(inner_frame, text=self.tr("volume"), font=("Times New Roman", 14), bg=self.bg_color, fg=self.fg_color).pack(pady=(20, 10))

        volume_scale = tk.Scale(inner_frame, from_=0, to=100, orient=tk.HORIZONTAL, length=280, font=("Times New Roman", 12), bg=self.bg_color, fg=self.fg_color, highlightthickness=0, troughcolor=self.btn_bg, activebackground=self.btn_active_bg)
        volume_scale.set(self.volume_level)
        volume_scale.pack()

        tk.Label(inner_frame, text=self.tr("theme"), font=("Times New Roman", 14), bg=self.bg_color, fg=self.fg_color).pack(pady=(30, 10))

        theme_var = tk.StringVar(value=self.theme)
        radio_frame = tk.Frame(inner_frame, bg=self.bg_color)
        radio_frame.pack()

        light_rb = tk.Radiobutton(radio_frame, text=self.tr("light_mode"), variable=theme_var, value="light", font=("Times New Roman", 12), bg=self.bg_color, fg=self.fg_color, selectcolor=self.btn_bg, activebackground=self.bg_color)
        dark_rb = tk.Radiobutton(radio_frame, text=self.tr("dark_mode"), variable=theme_var, value="dark", font=("Times New Roman", 12), bg=self.bg_color, fg=self.fg_color, selectcolor=self.btn_bg, activebackground=self.bg_color)
        light_rb.pack(anchor="w", padx=60, pady=2)
        dark_rb.pack(anchor="w", padx=60, pady=2)

        tk.Label(inner_frame, text=self.tr("language"), font=("Times New Roman", 14), bg=self.bg_color, fg=self.fg_color).pack(pady=(30, 10))

        language_var = tk.StringVar(value=self.language)
        lang_frame = tk.Frame(inner_frame, bg=self.bg_color)
        lang_frame.pack()

        tk.Radiobutton(lang_frame, text=self.tr("english"), variable=language_var, value="en", font=("Times New Roman", 12), bg=self.bg_color, fg=self.fg_color, selectcolor=self.btn_bg, activebackground=self.bg_color).pack(side=tk.LEFT, padx=20)
        tk.Radiobutton(lang_frame, text=self.tr("chinese"), variable=language_var, value="zh", font=("Times New Roman", 12), bg=self.bg_color, fg=self.fg_color, selectcolor=self.btn_bg, activebackground=self.bg_color).pack(side=tk.LEFT, padx=20)

        tk.Label(inner_frame, text=self.tr("mqtt_broker"), font=("Times New Roman", 14), bg=self.bg_color, fg=self.fg_color).pack(pady=(30, 10))

        mqtt_entry = tk.Entry(inner_frame, font=("Times New Roman", 12), bg="white" if self.theme == "light" else "#333333", fg="black" if self.theme == "light" else "#dddddd", insertbackground=self.text_fg)
        mqtt_entry.insert(0, self.mqtt_broker)
        mqtt_entry.pack()

        error_label = tk.Label(inner_frame, text="", font=("Times New Roman", 12), fg="red", bg=self.bg_color)
        error_label.pack()
        ...
        save_btn = tk.Button(buttons_frame, text=self.tr("save"), font=("Times New Roman", 12, "bold"), bg=self.fg_color, fg=self.bg_color, activebackground=self.btn_active_bg, command=save_settings, relief="raised", bd=3, padx=10)
        save_btn.pack(side=tk.LEFT, padx=10)
        cancel_btn = tk.Button(buttons_frame, text=self.tr("cancel"), font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, relief="raised", bd=2, command=settings_win.destroy)
    ...
    def login_prompt(self):
        ...
        if dialog.result:
            ...
            if password == HARDCODED_PASSWORD:
                ...
                messagebox.showerror("Error", self.tr("incorrect"))
    ...
    def staff_info(self):
        ...
        self.center_window(prompt_win, width, height)
        border_frame = tk.Frame(prompt_win, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("staff_info"), font=("Times New Roman", 18, "bold"), bg=self.bg_color, fg=self.fg_color)
        title.pack(pady=(20, 10))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 10))
        info_label = tk.Label(inner_frame, text=f"{self.tr('staff_in_charge')}: {self.current_user}", font=("Times New Roman", 14), bg=self.bg_color, fg=self.text_fg, wraplength=350)
        info_label.pack(pady=10)
        button_frame = tk.Frame(inner_frame, bg=self.bg_color)
        button_frame.pack(pady=(0, 20))
        ok_button = tk.Button(button_frame, text=self.tr("ok"), font=("Times New Roman", 12), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, command=prompt_win.destroy)
    ...
    def activity_log(self):
        ...
        self.center_window(log_window, 1000, 600)
        border_frame = tk.Frame(log_window, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("activity_log"), font=("Times New Roman", 20, "bold"), bg=self.bg_color, fg=self.fg_color)
        title.pack(pady=(20, 10))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 10))
        style = ttk.Style()
        style.theme_use("default")
        style.configure("Treeview", background=self.bg_color, fieldbackground=self.bg_color, foreground=self.text_fg, rowheight=30)
        style.configure("Treeview.Heading", font=("Times New Roman", 14, "bold"), background=self.btn_bg, foreground=self.fg_color)
        ...
        button_frame = tk.Frame(inner_frame, bg=self.bg_color)
        button_frame.pack(pady=(0, 20))
        cancel_btn = tk.Button(button_frame, text=self.tr("cancel"), font=("Times New Roman", 12), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, command=log_window.destroy)
        cancel_btn.pack()
    ...
    def about_system(self):
        ...
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both", padx=4, pady=4)
        header = tk.Label(inner_frame, text="🛒 About the Supermarket System", font=("Times New Roman", 18, "bold"), bg="#007c30", fg="white", pady=10)
        ...
        label = tk.Label(inner_frame, text=content, font=("Times New Roman", 12), bg="#e8f5e9", fg="#333333", justify="left", anchor="nw", padx=20, pady=20)
        label.pack(fill=tk.BOTH, expand=True)
        close_btn = tk.Button(inner_frame, text="Close", command=about_win.destroy, font=("Times New Roman", 12, "bold"), bg="#007c30", fg="white", padx=10, pady=5, activebackground="#005c24", relief="flat")
        close_btn.pack(pady=10)

    def live_camera(self):
        self.start_barcode_detection()
    ...
    def create_tip_box(self, parent):
        ...
        tip_label = tk.Label(parent, text=next(tips), bg="white", fg="#333333", font=("Times New Roman", 18, "italic"), wraplength=700, justify="center")
    ...
    def prompt_shutdown_password(self):
        ...
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("shutdown"), font=("Times New Roman", 16, "bold"), bg=self.bg_color, fg=self.fg_color)
        title.pack(pady=(20, 10))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 10))
        password_entry = tk.Entry(inner_frame, show="*", font=("Times New Roman", 14), bg="white" if self.theme == "light" else "#333333", fg="black" if self.theme == "light" else "#dddddd", insertbackground=self.text_fg, relief="solid", bd=1, width=25)
        ...
        buttons_frame = tk.Frame(inner_frame, bg=self.bg_color)
        buttons_frame.pack(pady=10)
        submit_btn = tk.Button(buttons_frame, text=self.tr("submit"), font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, relief="raised", bd=2, command=check_password)
        submit_btn.pack(side=tk.LEFT, padx=10)
        cancel_btn = tk.Button(buttons_frame, text=self.tr("cancel"), font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, relief="raised", bd=2, command=prompt_win.destroy)
        cancel_btn.pack(side=tk.LEFT, padx=10)
    ...
    def shutdown(self):
        ...
        width, height = 420, 220
        self.center_window(confirm_win, width, height)
        border_frame = tk.Frame(confirm_win, bg=border_color)
        border_frame.pack(expand=True, fill="both", padx=4, pady=4)
        inner_frame = tk.Frame(border_frame, bg=self.bg_color)
        inner_frame.pack(expand=True, fill="both")
        title = tk.Label(inner_frame, text=self.tr("confirm"), font=("Times New Roman", 20, "bold"), bg=self.bg_color, fg=self.fg_color)
        title.pack(pady=(20, 10))
        separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
        separator.pack(fill='x', padx=20, pady=(0, 20))
        tk.Label(inner_frame, text=self.tr("AYS"), font=("Times New Roman", 14), bg=self.bg_color, fg=self.text_fg, wraplength=360, justify="center").pack(pady=(0, 40))
        buttons_frame = tk.Frame(inner_frame, bg=self.bg_color)
        buttons_frame.pack()
        yes_btn = tk.Button(buttons_frame, text="Yes", font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, relief="raised", bd=2, width=10, command=shutdown_yes)
        yes_btn.pack(side=tk.LEFT, padx=10)
        no_btn = tk.Button(buttons_frame, text="No", font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, relief="raised", bd=2, width=10, command=shutdown_no)
    ...
    def start_mqtt(self):
        def on_connect(client, userdata, flags, reason_code, properties):
            if reason_code.is_failure:
                ...
                messagebox.showerror("MQTT Error", self.tr("mqtt_error"))
                self.publisher_active = False
            else:
                print("Connected to MQTT broker")
                client.subscribe([(MQTT_TOPIC_DOME, 0), (MQTT_TOPIC_RIBBON, 0)])
                self.publisher_active = True

        def on_message(client, userdata, msg):
            try:
                message = msg.payload.decode()
                if msg.topic == MQTT_TOPIC_DOME and message.startswith("Object Count:"):
                    count = int(message.split(":")[1].strip())
                    self.dome_count.set(count)
                elif msg.topic == MQTT_TOPIC_RIBBON and message.startswith("Number Count:"):
                    count = int(message.split(":")[1].strip())
                    self.ribbon_count.set(count)
                else:
                    print(f"Unknown MQTT message format or topic: {msg.topic} - {message}")
                self.last_message_time = datetime.datetime.now()
            except ValueError as e:
                print(f"Invalid MQTT message received on {msg.topic}: {message}, error: {e}")

        self.mqtt_client = mqtt.Client(protocol=mqtt.MQTTv5)
        self.mqtt_client.on_connect = on_connect
        self.mqtt_client.on_message = on_message
        try:
            self.mqtt_client.connect(self.mqtt_broker, MQTT_PORT, keepalive=60)
    ...
    def start(self):
        ...
        live_counts_win.overrideredirect(True)

        def check_counts():
            if not self.publisher_active or not self.last_message_time:
                return
            if (datetime.datetime.now() - self.last_message_time).total_seconds() > 10:
                self.publisher_active = False
                return
            if self.dome_count.get() != self.ribbon_count.get():
                try:
                    conn = mariadb.connect(**DB_CONFIG)
                    cursor = conn.cursor()
                    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    description = "unusual behaviour detected"
                    cursor.execute("INSERT INTO activity_log (timestamp, description) VALUES (%s, %s)", (timestamp, description))
                    conn.commit()
                    conn.close()
                    print("Logged unusual behaviour due to count mismatch")
                    messagebox.showinfo("Activity Log", self.tr("log_recorded"))
                except mariadb.Error as err:
                    print(f"Failed to log count mismatch: {err}")
                    messagebox.showerror("Database Error", f"Failed to log count mismatch: {err}")

        def update_counts():
            if self.publisher_active:
                check_counts()
            live_counts_win.after(10000, update_counts)
        ...
        def after_count_init():
            ...
            try:
                barcode_blocked_img = Image.open("/home/pi/hailo-rpi5-examples/images/barcode_blocked.jpeg")
                multiple_barcode_img = Image.open("/home/pi/hailo-rpi5-examples/images/multiple_barcode.jpeg")
                camera_blocked_img = Image.open("/home/pi/hailo-rpi5-examples/images/camera_blocked.jpeg")
                barcode_detected_img = Image.open("/home/pi/hailo-rpi5-examples/images/barcode_detected.jpeg")
                barcode_blocked_img = barcode_blocked_img.resize((barcode_blocked_img.width // 2, barcode_blocked_img.height // 2))
                multiple_barcode_img = multiple_barcode_img.resize((multiple_barcode_img.width // 2, multiple_barcode_img.height // 2))
                camera_blocked_img = camera_blocked_img.resize((camera_blocked_img.width // 2, camera_blocked_img.height // 2))
                barcode_detected_img = barcode_detected_img.resize((barcode_detected_img.width // 2, barcode_detected_img.height // 2))
                barcode_blocked_img = ImageTk.PhotoImage(barcode_blocked_img)
                multiple_barcode_img = ImageTk.PhotoImage(multiple_barcode_img)
                camera_blocked_img = ImageTk.PhotoImage(camera_blocked_img)
                barcode_detected_img = ImageTk.PhotoImage(barcode_detected_img)
            ...
            def flash_green_tick(text, img):
                ...
                flash_label.config(text=text, font=("Times New Roman", 48, "bold"), fg="white", bg="#00ff00")
                flash_image_label.config(image=img, bg="#00ff00")
                self.flash_screen.deiconify()
                self.flash_screen.after(1000, lambda: [self.flash_screen.withdraw(), live_counts_win.lift(), live_counts_win.focus_force()])

            def flash_red_screen(text, img):
                self.flash_screen.configure(bg="#ff0000")
                flash_label.config(text=text, font=("Times New Roman", 48, "bold"), fg="white", bg="#ff0000")
                flash_image_label.config(image=img, bg="#ff0000")
                self.flash_screen.deiconify()
                self.flash_screen.after(1500, lambda: [self.flash_screen.withdraw(), live_counts_win.lift(), live_counts_win.focus_force()])
            ...
            def simulate_flash_events():
                ...
                func()
                self.flash_loop_id["id"] = self.after(5000, simulate_flash_events)

            def on_live_counts_close():
                if self.flash_loop_id["id"] is not None:
        ...
        def exit_fullscreen():
            ...
            inner_frame = tk.Frame(border_frame, bg=self.bg_color)
            inner_frame.pack(expand=True, fill="both", padx=4, pady=4)
            title = tk.Label(inner_frame, text=self.tr("passreq"), font=("Times New Roman", 20, "bold"), bg=self.bg_color, fg=self.fg_color)
            title.pack(pady=(20, 10))
            separator = tk.Frame(inner_frame, height=2, bg=self.fg_color, bd=0, relief='flat')
            separator.pack(fill='x', padx=20, pady=(0, 10))
            password_entry = tk.Entry(inner_frame, show="*", font=("Times New Roman", 14), bg="white" if self.theme == "light" else "#333333", fg="black" if self.theme == "light" else "#dddddd", insertbackground=self.text_fg, relief="solid", bd=1)
            password_entry.pack(pady=10)
            password_entry.focus()
            ...
            buttons_frame = tk.Frame(inner_frame, bg=self.bg_color)
            buttons_frame.pack(pady=10)
            tk.Button(buttons_frame, text=self.tr("submit"), font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, bd=2, command=check_password).pack(side=tk.LEFT, padx=10)
            tk.Button(buttons_frame, text=self.tr("cancel"), font=("Times New Roman", 12, "bold"), bg=self.btn_bg, fg=self.fg_color, activebackground=self.btn_active_bg, bd=2, command=prompt_win.destroy).pack(side=tk.LEFT, padx=10)
            prompt_win.bind("<Return>", lambda e: check_password())

        exit_btn = tk.Button(live_counts_win, text=self.tr("exit"), font=("Times New Roman", 14, "bold"), bg="#e8f5e9", activebackground="#c8e6c9", relief="raised", bd=3, command=exit_fullscreen)
