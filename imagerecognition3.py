...
# Database configuration (same as in SupermarketApp)
DB_CONFIG = {
    "host": "localhost",
    "user": "root",
    "password": "root",
...
class ImageRecognitionApp:
    def __init__(self, parent=None, picam2=None, mqtt_broker="192.168.43.101", mqtt_port=1883):
        self.parent = parent
    ...
    def save_summary_to_db(self, summary_value):
        """Save the detected summary value to the activity_log table."""
        try:
            conn = mariadb.connect(**DB_CONFIG)
            cursor = conn.cursor()
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            description = f"Summary Value Detected: {summary_value}"
            image_path = ""  # No image saved in this case
            cursor.execute(
            ...
            conn.commit()
            conn.close()
            print(f"Saved to activity_log: {description}")
        except mariadb.Error as err:
            print(f"Error saving summary to database: {err}")
            messagebox.showerror("Database Error", f"Failed to save summary to database: {err}")
    ...
    def update_frame(self):
        ...
        if self.detection_started:
            ...
            if not found_summary or summary_value is None:
                ...
                elif time.time() - self.non_detection_start_time >= 2:
        ...
        self.label.after(100, self.update_frame)
