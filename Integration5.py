...
class SupermarketApp(tk.Tk):
    ...
    def show_mismatch_popup(self, live_counts_win, dome, ribbon):
        ...
        popup_win.transient(live_counts_win)  # Bind popup to live_counts_win
        popup_win.grab_set()
        popup_win.resizable(False, False)
        popup_win.overrideredirect(True)
        popup_win.configure(bg=border_color)
        width, height = 400, 200
        # Center popup relative to live_counts_win
        ...
        popup_win.after(3000, popup_win.destroy)  # Auto-close after 3 seconds
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                if msg.topic == MQTT_TOPIC_BARCODE:
                    ...
                    print(f"message received is {text_key}")
                    if not text_key:
                        return
        ...
        def update_counts():
            ...
            with self.number_lock:
                ...
                if dome != ribbon:
                    ...
                    elif (current_time - self.mismatch_start_time) >= 6:
                        try:
                            conn = mariadb.connect(**DB_CONFIG)
                            cursor = conn.cursor()
                            cursor.execute(
                                "INSERT INTO activity_log (timestamp, description, image_path) VALUES (NOW(), ?, NULL)",
                                (f"Mismatch detected: dome_count={dome}, ribbon_count={ribbon}",)
                            )
                            conn.commit()
                            conn.close()
                            self.show_mismatch_popup(live_counts_win, dome, ribbon)
                            self.mismatch_start_time = current_time
                        except mariadb.Error as err:
