...
DEBOUNCE_COUNT = 5  # Number of consistent detections before confirming
...
# Create shutdown button first
    ...
    def detection_loop(self):
        while self.running:
            ...
            preprocessed_roi = self.preprocess_for_ocr(roi)

            self.ocr_queue.put(preprocessed_roi)
        ...
        while True:
            ...
            if detected_text:
                ...
                if len(self.recent_detections) >= DEBOUNCE_COUNT:
                    ...
                    if most_common[1] >= DEBOUNCE_COUNT - 1:
                        print(f"✅ Confirmed Number: {most_common[0]}")
            else:
                        ...
                        if detected_text:
                            ...
                            if len(self.recent_detections) == DEBOUNCE_COUNT:
                                if all(text == self.recent_detections[0] for text in self.recent_detections):
                                    print(f"✅ Confirmed Number: {self.recent_detections[0]}")
                        else:
                            self.recent_detections.clear()
    ...
    def dummy_yolo_detector(self, frame):
        # Dummy detector to simulate detection on full frame
        height, width = frame.shape[:2]
        box_w, box_h = width // 4, height // 8
        x1 = width // 2 - box_w // 2
        y1 = height // 2 - box_h // 2
        x2 = x1 + box_w
        y2 = y1 + box_h
    ...
    def main(self):
        ...
        while True:
            ret, frame = cap.read()  # Read full frame here!
            if not ret:
                break

            if USE_FIXED_ROI:
                roi = frame[ROI_Y1:ROI_Y2, ROI_X1:ROI_X2]
                x1_exp, y1_exp, x2_exp, y2_exp = ROI_X1, ROI_Y1, ROI_X2, ROI_Y2
            else:
                detections = self.dummy_yolo_detector(frame)  # Detect on full frame
                if not detections:
                    continue
            ...
            self.detected_number = detected_text if detected_text else "0"
            # Debounce filter: keep last N results
            if detected_text:
                recent_detections.append(detected_text)
                if len(self.recent_detections) >= DEBOUNCE_COUNT:
                    most_common = Counter(self.recent_detections).most_common(1)[0]  # (number, frequency)
                    if most_common[1] >= DEBOUNCE_COUNT - 1:  # 3 out of 4 or 4 out of 5, etc.
                        print(f"✅ Confirmed Number: {most_common[0]}")
            else:
                recent_detections.clear()

            # Draw box and detected text ON THE FULL FRAME (not on preprocessed_roi!)
            cv2.rectangle(frame, (x1_exp, y1_exp), (x2_exp, y2_exp), (0, 255, 0), 2)
            cv2.putText(frame, detected_text, (x1_exp, y1_exp - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            # Show the windows:
            # Get full frame size
            # Show full frame with bounding box and text
            cv2.imshow('Full Frame with Box', frame)

            # Show resized preprocessed ROI (zoomed in view)
            frame_height, frame_width = frame.shape[:2]
            resized_roi = cv2.resize(preprocessed_roi, (frame_width, frame_height), interpolation=cv2.INTER_LINEAR)
            cv2.imshow('Preprocessed ROI (Zoomed)', resized_roi)
    ...
    def start(self):
        ...
        def update_counts():
            ...
            try:
                detected_num = int(self.detected_number)  # assume self.detected_number exists and is a string or int
...
# Convert the images to a format Tkinter can work with
