...
class SupermarketApp(tk.Tk):
    ...
    def simulate_activity_log(self):
        try:
            ...
            messagebox.showerror("Database Error", f"Failed to insert simulated Levi log: {err}")
            ...
            if frame is not None:
                ...
                if detections:
                    ...
                    if label == 'number' and conf > 0.5:
                        ...
                        processed = self.preprocess_for_ocr(roi)
                        results = self.reader.readtext(processed, allowlist='0123456789', min_size=3, text_threshold=0.4, adjust_contrast=0.6)
                        ...
                        processed_resized = cv2.resize(processed, (300, 230))
                        img_proc = Image.fromarray(processed_resized)
    ...
    def preprocess_for_ocr(self, image):
        ...
        # Since we're keeping color, convert to grayscale only for thresholding
