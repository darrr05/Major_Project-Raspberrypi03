...
translations = {
    "en": {
        ...
        "warning": "Warning",
        "select_log_first": "Please select a log entry first."
    },
    "zh": {
        "settings": "设置",
        "volume": "扬声器音量：",
...
class LoginDialog(simpledialog.Dialog):
    def __init__(self, parent, title, tr_func):
        self.tr = tr_func
...
class SupermarketApp(tk.Tk):
    def __init__(self):
        ...
        # Run separate_modbus2.py in a separate terminal window
        #self.run_separate_modbus2()
        ...
        self.flash_image_label = None
        self.flash_label = None
        self.is_flash_paused = False
                    ...
                    if isinstance(subchild, tk.Label):
                        ...
                        for nested in subchild.winfo_children():
                            if isinstance(nested, tk.Label):
                                ...
                                nested.config(bg="white" if self.theme == "light" else "#333333",
    ...
    def create_widgets(self):
        ...
        self.shutdown_btn = tk.Button(self.top_bar, image=shutdown_img_tk, font=("Times New Roman", 20),
                                     command=self.prompt_shutdown_password, fg="black", bg="white", bd=0, activebackground="lightgray")
    ...
    def run_separate_modbus2(self):
        """Run separate_modbus2.py in a separate terminal window with necessary setup"""
        try:
            # Full command to cd into the directory, source the setup_env.sh, and run the script
            cmd = (
                "cd ~/hailo-rpi5-examples && "
                "source setup_env.sh && "
                "python basic_pipelines/separate_modbus2.py"
            )

            # Run the command in a new terminal window
            subprocess.Popen(["bash", "-c", cmd])
            print("separate_modbus2.py launched successfully.")
        except Exception as e:
            print(f"Error launching separate_modbus2.py: {e}")
    ...
    def login_prompt(self):
        dialog = LoginDialog(self, "Login", self.tr)
        if dialog.result:
    ...
    def activity_log(self):
        ...
        try:
            conn = mariadb.connect(**DB_CONFIG)
            cursor = conn.cursor()
            cursor.execute("SELECT id, timestamp, description, image_path FROM activity_log ORDER BY timestamp DESC")
        ...
        columns = ("ID", "Time", "Description", "Image")
        ...
        for log_id, timestamp, desc, image_path in records:
            img = image_path if image_path else "nil"
            tree.insert("", "end", values=(log_id, timestamp, desc, img))
    ...
    def start_mqtt(self):
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                elif message.startswith("Number Count:"):
                    count = int(message.split(":")[1].strip())
    ...
    def start(self):
        ...
        def check_counts():
            ...
            if self.dome_count.get() != self.ribbon_count.get():
                self.is_flash_paused = True
                try:
                    conn = mariadb.connect(**DB_CONFIG)
                    cursor = conn.cursor()
                    timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    description = "Unusual behaviour detected: Count mismatch"
                    cursor.execute(
                        "INSERT INTO activity_log (timestamp, description) VALUES (%s, %s)",
                    ...
                    alert_win.configure(bg="white")

                    width, height = 400, 200
                    x = (alert_win.winfo_screenwidth() // 2) - (width // 2)
                    y = (alert_win.winfo_screenheight() // 2) - (height // 2)
                    alert_win.geometry(f"{width}x{height}+{x}+{y}")

                    border_frame = tk.Frame(alert_win, bg="white")
                    border_frame.pack(expand=True, fill="both", padx=4, pady=4)

                    inner_frame = tk.Frame(border_frame, bg=self.bg_color)
                    inner_frame.pack(expand=True, fill="both")

                    tk.Label(inner_frame, text=self.tr("log_recorded"),
                            font=("Times New Roman", 14), bg=self.bg_color, fg=self.text_fg,
                            wraplength=360, justify="center").pack(pady=(20, 20))

                    ok_btn = tk.Button(inner_frame, text=self.tr("ok"), font=("Times New Roman", 12, "bold"),
                                      bg=self.btn_bg, fg=self.fg_color,
                                      activebackground=self.btn_active_bg,
                                      command=lambda: [alert_win.destroy(), setattr(self, 'is_flash_paused', False)])
                    ok_btn.pack(pady=10)
        ...
        def after_count_init():
            ...
            self.flash_image_label = tk.Label(self.flash_screen, bg="white")
            self.flash_image_label.pack(pady=(100, 20))

            self.flash_label = tk.Label(self.flash_screen, bg="white")
            ...
            def flash_green_tick(text, img):
                ...
                self.flash_label.config(text=text,
                                 ...
                                 fg="white", bg="#00ff00")
                self.flash_image_label.config(image=img, bg="#00ff00")
                self.flash_screen.deiconify()
                self.flash_screen.after(1000, lambda: [
                    self.flash_screen.withdraw(),
            ...
            def flash_red_screen(text, img):
                ...
                self.flash_label.config(text=text,
                                 font=("Times New Roman", 48, "bold"),
                                 fg="white", bg="#ff0000")
                self.flash_image_label.config(image=img, bg="#ff0000")
                self.flash_screen.deiconify()
                self.flash_screen.after(1500, lambda: [
                    self.flash_screen.withdraw(),
                    live_counts_win.lift(),
                    live_counts_win.focus_force()
                ])
            ...
            def simulate_flash_events():
                if not live_counts_win.winfo_exists() or self.is_flash_paused:
            ...
            def on_live_counts_close():
                if self.flash_loop_id and self.flash_loop_id["id"] is not None:
